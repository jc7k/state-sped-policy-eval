...
class TestDataCollectionPipeline:
    ...
    def test_naep_collection_end_to_end(self, mock_sleep, mock_get, 
                                       sample_naep_api_response, temp_data_dir):
...
class TestCrossModuleValidation:
    ...
    def test_consistent_state_codes_across_sources(self):
        ...
        naep_data = pd.DataFrame({
            'state': ['AL', 'CA', 'TX'],
            'naep_score': [245, 252, 248]
        })
        
        edfacts_data = pd.DataFrame({
            'state': ['AL', 'CA', 'TX'], 
            'inclusion_rate': [45.2, 52.1, 48.7]
        })
        
        census_data = pd.DataFrame({
            'state': ['AL', 'CA', 'TX'],
            'expenditure': [8500, 85000, 67000]
        })
