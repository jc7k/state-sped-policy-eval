...
class CensusDataParser:
    ...
    def _clean_f33_data(self, df: pd.DataFrame, year: int) -> pd.DataFrame:
        ...
        for idx, row in df.iterrows():
            ...
            if first_col.lower() in ['', 'nan', 'total', 'united states', 'us', 'geographic area']:
                continue
        ...
        for idx, row in df.iterrows():
            ...
            # Skip invalid rows
            if not state_name or state_name.lower() in ['', 'nan', 'total', 'united states', 'us']:
                continue
            ...
            if 'instruction_expenditure' in df.columns:
                row_data['instruction_expenditure'] = self._parse_numeric(row.get('instruction_expenditure'))
                
            if 'support_services' in df.columns:
                row_data['support_services_expenditure'] = self._parse_numeric(row.get('support_services'))
    ...
    def parse_all_files(self, data_dir: Path) -> pd.DataFrame:
        ...
        self.logger.info(f"Combined Census data: {len(df_combined)} total records across {df_combined['year'].nunique()} years")
    ...
    def validate_parsed_data(self, df: pd.DataFrame) -> Dict:
        ...
        for year in df['year'].unique():
            ...
            if missing:
                validation['missing_states_by_year'][int(year)] = sorted(list(missing))
...
def main():
    ...
    if args.output:
        ...
        output_path = data_dir / 'census_education_finance_parsed.csv'
