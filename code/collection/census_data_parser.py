    ...
    def _clean_f33_data(self, df: pd.DataFrame, year: int) -> pd.DataFrame:
        ...
        for _idx, row in df.iterrows():
            ...
            if "total_revenue" in df.columns:
                row_data["total_revenue"] = self._parse_numeric(
                    row.get("total_revenue")
                )

            if "federal_revenue" in df.columns:
                row_data["federal_revenue"] = self._parse_numeric(
                    row.get("federal_revenue")
                )

            if "state_revenue" in df.columns:
                row_data["state_revenue"] = self._parse_numeric(
                    row.get("state_revenue")
                )

            if "local_revenue" in df.columns:
                row_data["local_revenue"] = self._parse_numeric(
                    row.get("local_revenue")
                )

            if "total_expenditure" in df.columns:
                row_data["total_expenditure"] = self._parse_numeric(
    ...
    def parse_all_files(self, data_dir: Path) -> pd.DataFrame:
        ...
        for file_path in sorted(census_files):
            ...
            try:
                ...
                self.logger.warning(
                    f"Could not extract year from filename: {file_path}"
                )
                continue
            ...
            try:
                ...
                if not df_year.empty:
                    ...
                    self.logger.info(
                        f"Successfully parsed {len(df_year)} records for {year}"
                    )
            except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parse Census F-33 education finance data"
    )
    parser.add_argument(
        "--data-dir", type=Path, help="Directory containing Census Excel files"
    )
    parser.add_argument("--output", type=Path, help="Output CSV file path")
    parser.add_argument(
        "--validate", action="store_true", help="Run validation on parsed data"
    )
