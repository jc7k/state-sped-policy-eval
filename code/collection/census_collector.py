...
class CensusEducationFinance:
    ...
    def fetch_state_finance(self, years: list[int]) -> pd.DataFrame:
        ...
        for i, year in enumerate(years):
            ...
            if year >= 2013:
                ...
                endpoint = (
                    f"{self.base_url}/{year}/governments/school-districts/finance"
                )
    ...
    def _parse_finance_record(self, row_data: dict, year: int) -> dict | None:
        ...
        try:
            ...
            if (
                record["total_expenditures"]
                and record["enrollment"]
                and record["enrollment"] > 0
            ):
                record["per_pupil_spending"] = (
                    record["total_expenditures"] / record["enrollment"]
                )
            else:
    ...
    def validate_data(self, df: pd.DataFrame) -> dict:
        ...
        validation = {
            ...
            "missing_enrollment": (
                ...
                if len(df) > 0 and "enrollment" in df.columns
                else 0
            ),
            "passed": True,
            "errors": [],
        ...
        if len(df) > 0:
            ...
            if duplicates > 0:
                validation["errors"].append(
                    f"{duplicates} duplicate state-year combinations found"
                )
                validation["passed"] = False
        ...
        if len(df) > 0 and "total_expenditures" in df.columns:
            ...
            if missing_rate > 0.2:
                validation["warnings"].append(
                    f"High missing expenditure rate: {missing_rate:.1%}"
                )

        return validation

    def save_data(
        self, df: pd.DataFrame, output_path: str = "data/raw/census_raw.csv"
    ) -> bool:
        """
