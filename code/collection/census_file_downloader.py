    ...
    def download_education_finance_data(
        ...
        for i, year in enumerate(years):
            if year not in self.data_urls:
                ...
                results["failed_downloads"].append({"year": year, "reason": "No URLs available"})
                continue
            ...
            for file_name in year_data["files"]:
                ...
                output_file = output_dir / f"census_education_finance_{year}_{file_name.replace('.html', '.html')}"
                ...
                try:
                    ...
                    self.logger.info(f"Successfully downloaded {file_name} for {year} ({len(response.content)} bytes)")
        ...
        for i, url in enumerate(csv_urls):
            try:
                ...
                try:
                    ...
                    self.logger.warning(f"Downloaded file {filename} is not a valid CSV")
