...
def calculate_confidence_intervals(
    ...
    std_errors: Optional[pd.Series] = None,
    ...
    if std_errors is None or pd.isna(std_errors).all():
        ...
        return pd.Series(coef_array - margin, index=coefficients.index), \
               pd.Series(coef_array + margin, index=coefficients.index)
...
def save_figure(
    ...
    format_params = {
        ...
        'pdf': {'dpi': dpi, 'bbox_inches': bbox_inches, 'metadata': {'Creator': 'Visualization Module'}},
...
def add_significance_stars(
    ax: plt.Axes,
    x_pos: Union[float, List[float], np.ndarray],
    y_pos: Union[float, List[float], np.ndarray], 
    ...
    stars_positions = [(x, y + offset, _get_significance_stars(p)) 
                      for x, y, p in zip(x_array, y_array, p_array)
