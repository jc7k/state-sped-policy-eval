...
class TreatmentEffectsDashboard:
    ...
    def __init__(self,
        ...
        print(f"TreatmentEffectsDashboard initialized:")
    ...
    def _load_treatment_effects(self) -> Dict[str, pd.DataFrame]:
        ...
        for file_path in agg_files:
            outcome = file_path.name.replace("aggregated_effects_", "").replace(".csv", "")
    ...
    def _get_state_coordinates(self) -> Dict[str, Tuple[float, float]]:
        ...
        coords = {
            'AL': (-86.8, 32.8), 'AK': (-152.0, 64.0), 'AZ': (-111.9, 34.2),
            'AR': (-92.2, 34.8), 'CA': (-119.8, 36.8), 'CO': (-105.5, 39.2),
            'CT': (-72.7, 41.6), 'DE': (-75.5, 39.2), 'DC': (-77.0, 38.9),
            'FL': (-81.5, 27.8), 'GA': (-83.2, 32.2), 'HI': (-157.8, 21.3),
            'ID': (-114.6, 44.1), 'IL': (-89.2, 40.1), 'IN': (-86.3, 39.8),
            'IA': (-93.6, 42.0), 'KS': (-98.4, 38.5), 'KY': (-84.9, 37.8),
            'LA': (-91.8, 31.2), 'ME': (-69.2, 45.2), 'MD': (-76.5, 39.0),
            'MA': (-71.8, 42.4), 'MI': (-84.5, 43.3), 'MN': (-94.6, 46.4),
            'MS': (-89.4, 32.7), 'MO': (-92.2, 38.3), 'MT': (-110.4, 47.1),
            'NE': (-99.8, 41.5), 'NV': (-117.0, 39.8), 'NH': (-71.5, 43.2),
            'NJ': (-74.8, 40.2), 'NM': (-106.2, 34.5), 'NY': (-74.2, 42.2),
            'NC': (-78.6, 35.8), 'ND': (-99.8, 47.5), 'OH': (-82.7, 40.2),
            'OK': (-97.1, 35.6), 'OR': (-122.0, 44.9), 'PA': (-77.2, 40.3),
            'RI': (-71.4, 41.6), 'SC': (-80.9, 33.8), 'SD': (-99.9, 44.3),
            'TN': (-86.4, 35.9), 'TX': (-97.7, 31.1), 'UT': (-111.9, 40.2),
            'VT': (-72.6, 44.0), 'VA': (-78.2, 37.7), 'WA': (-121.5, 47.4),
    ...
    def _define_regions(self) -> Dict[str, List[str]]:
        ...
        regions = {
            ...
            'South': ['AL', 'AR', 'DE', 'DC', 'FL', 'GA', 'KY', 'LA', 'MD', 
    ...
    def create_treatment_timeline_map(self,
        ...
        treated_states = self.policy_data[self.policy_data['post_treatment'] == 1].copy()
        ...
        year_color_map = dict(zip(treatment_years, colors))
        ...
        for state, coord in self.state_coords.items():
            ...
            if not state_reform.empty:
                ...
                marker = 'o'
                size = 150
                label = f'{state} ({reform_year})'
            else:
                color = 'lightgray'
                marker = 's'
                size = 80
                label = f'{state} (No reform)'
            
            ax.scatter(lon, lat, c=[color], s=size, marker=marker, 
                      edgecolors='black', linewidth=1, alpha=0.8)
            
            # Add state labels
            ax.annotate(state, (lon, lat), xytext=(5, 5), 
                       textcoords='offset points', fontsize=8, fontweight='bold')
        
        # Create legend for treatment years
        legend_elements = []
        for year in treatment_years:
            legend_elements.append(plt.scatter([], [], c=[year_color_map[year]], 
                                             s=150, marker='o', edgecolors='black',
                                             label=f'Reformed {year}'))
        legend_elements.append(plt.scatter([], [], c='lightgray', s=80, marker='s',
                                         edgecolors='black', label='No reform'))
        
        ax.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(1.02, 1),
                 title='Policy Reform Year', title_fontsize=12)
    ...
    def create_regional_comparison_chart(self,
        ...
        treated_states = self.policy_data[self.policy_data['post_treatment'] == 1]['state'].unique()
        ...
        bars1 = ax1.bar(regional_df['region'], regional_df['treatment_rate'], 
                        color=['steelblue', 'darkorange', 'forestgreen', 'crimson'],
                        alpha=0.7, edgecolor='black')
        
        ax1.set_title('Policy Reform Adoption by Region', fontsize=14, fontweight='bold')
        ...
        for i, bar in enumerate(bars1):
            ...
            ax1.text(bar.get_x() + bar.get_width()/2., height + 0.02,
                    f'{treated}/{total}', ha='center', va='bottom', fontweight='bold')
        ...
        if outcome in self.treatment_effects:
            ...
            if 'simple_att' in effect_data.columns:
                ...
                overall_se = effect_data.get('simple_se', [0]).iloc[0]
                ...
                bars2 = ax2.bar(regions_list, effects, 
                               color=['steelblue', 'darkorange', 'forestgreen', 'crimson'],
                               alpha=0.7, edgecolor='black')
                
                ax2.axhline(y=0, color='black', linestyle='-', alpha=0.8)
                ax2.set_title(f'Treatment Effects by Region\\n({self._format_outcome_label(outcome)})', 
    ...
    def create_policy_type_analysis(self,
        ...
        bars1 = ax1.bar(reform_years.index, reform_years.values, 
        ...
        intervention_counts = {
            ...
            'Federal Monitoring': len(monitoring_data['state'].unique()) if not monitoring_data.empty else 0,
            'Court Orders': len(court_data['state'].unique()) if not court_data.empty else 0
        }
        
        bars2 = ax2.bar(intervention_counts.keys(), intervention_counts.values(),
                       color=['steelblue', 'darkorange', 'forestgreen'],
                       alpha=0.7, edgecolor='black')
        
        ax2.set_title('Types of Policy Interventions (2009-2023)', fontsize=14, fontweight='bold')
    ...
    def create_complete_dashboard(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            for outcome in self.treatment_effects.keys():
