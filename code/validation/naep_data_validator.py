...
class NAEPDataValidator:
    ...
    def validate_dataset(self, data_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_results['summary']['issues'].append(f"Validation error: {str(e)}")
    ...
    def _validate_completeness(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        completeness['coverage_by_dimension'] = {
            'states': {
                ...
                'unexpected': list(set(df['state'].unique()) - set(self.expected_states))
            },
            'years': {
            ...
            'grades': {
                ...
                'unexpected': list(set(df['grade'].unique()) - set(self.expected_grades))
            },
            'subjects': {
                'found': df['subject'].nunique(),
                'expected': len(self.expected_subjects),
                'missing': list(set(self.expected_subjects) - set(df['subject'].unique())),
                'unexpected': list(set(df['subject'].unique()) - set(self.expected_subjects))
            },
            'disability_statuses': {
                'found': df['disability_status'].nunique(),
                'expected': len(self.expected_disability_statuses),
                'missing': list(set(self.expected_disability_statuses) - set(df['disability_status'].unique())),
                'unexpected': list(set(df['disability_status'].unique()) - set(self.expected_disability_statuses))
            }
        }
                ...
                for grade in self.expected_grades:
                    for subject in self.expected_subjects:
                        for status in self.expected_disability_statuses:
                            expected_combinations.append((state, year, grade, subject, status))
        
        existing_combinations = set(
            zip(df['state'], df['year'], df['grade'], df['subject'], df['disability_status'])
        )
        ...
        for group_by in [None, 'year', 'grade', 'subject', 'state']:
            ...
            if group_by is None:
                ...
                for group_value in df[group_by].unique():
                    ...
                    swd_scores = group_data[group_data['disability_status'] == 'SWD']['mean_score']
                    non_swd_scores = group_data[group_data['disability_status'] == 'non-SWD']['mean_score']
    ...
    def _check_var_value_consistency(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        mapping_check = df.groupby(['var_value', 'disability_status']).size().reset_index(name='count')
        ...
        for _, row in mapping_check.iterrows():
            ...
            if var_val in expected_mappings:
                if expected_mappings[var_val] != status:
    ...
    def _generate_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if quality.get('outliers', {}).get('unreasonable_scores', {}).get('count', 0) > 0:
            summary['issues'].append(
        ...
        overall_gap = quality.get('achievement_gaps', {}).get('overall', {}).get('mean_gap')
        if overall_gap is not None:
            if overall_gap < 10 or overall_gap > 50:
                summary['warnings'].append(
    ...
    def generate_report(self, validation_results: Dict[str, Any], output_file: Path = None) -> str:
        ...
        report.append(f"Coverage: {completeness.get('coverage_percentage', 0):.1f}% ({completeness.get('total_records', 0):,}/{completeness.get('expected_records', 0):,} records)")
        ...
        if 'score_statistics' in quality:
            ...
            report.append(f"Range: {stats.get('min', 0):.1f} - {stats.get('max', 0):.1f}")
