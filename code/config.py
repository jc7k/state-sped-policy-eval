...
class Config:
    ...
    def __init__(self):
        ...
        self.edfacts_rate_limit_delay = float(
            os.getenv("EDFACTS_RATE_LIMIT_DELAY", "1.0")
        )
        self.ocr_rate_limit_delay = float(os.getenv("OCR_RATE_LIMIT_DELAY", "1.0"))

        # Data collection settings
        self.collection_years = self._parse_years(
            os.getenv(
                "COLLECTION_YEARS",
                "2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023",
            )
        )
        self.naep_years = self._parse_years(
            os.getenv("NAEP_YEARS", "2009,2011,2013,2015,2017,2019,2022")
        )
        self.ocr_years = self._parse_years(
            os.getenv("OCR_YEARS", "2009,2011,2013,2015,2017,2020")
        )
        self.naep_subjects = self._parse_list(
            os.getenv("NAEP_SUBJECTS", "mathematics,reading")
        )
        ...
        self.enable_validation = (
            os.getenv("ENABLE_VALIDATION", "true").lower() == "true"
        )
    ...
    def validate_configuration(self) -> dict[str, Any]:
        ...
        if not self.collection_years:
            validation["warnings"].append(
                "No collection years specified, using defaults"
            )
        ...
        if self.naep_rate_limit_delay < 1.0:
            validation["warnings"].append(
                "NAEP rate limit very low, may cause API blocking"
            )
