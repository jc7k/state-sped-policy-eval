...
class RobustnessTestingSuite:
    ...
    def __init__(self, 
        ...
        print(f"RobustnessTestingSuite initialized:")
        ...
        print(f"  Treated states: {self.data[self.data['post_treatment']==1]['state'].nunique()}")
    ...
    def leave_one_state_out_analysis(self, 
        ...
        baseline_att = baseline_result.get('simple_att', [np.nan])[0] if not baseline_result.empty else np.nan
        ...
        if save_results:
            output_path = self.results_dir / f"jackknife_analysis_{outcome}.csv"
            jackknife_df.to_csv(output_path, index=False)
            print(f"  Saved jackknife results: {output_path}")
        
        # Summary statistics
        if not jackknife_df.empty:
            valid_results = jackknife_df.dropna(subset=['att_estimate'])
            if not valid_results.empty:
                summary_stats = {
                    'mean_estimate': valid_results['att_estimate'].mean(),
                    'std_estimate': valid_results['att_estimate'].std(),
                    'min_estimate': valid_results['att_estimate'].min(),
                    'max_estimate': valid_results['att_estimate'].max(),
                    'baseline_estimate': baseline_att,
                    'max_influence': valid_results['influence'].abs().max(),
                    'influential_state': valid_results.loc[valid_results['influence'].abs().idxmax(), 'dropped_state']
                }
                
                print(f"  Jackknife Summary:")
                print(f"    Baseline ATT: {baseline_att:.3f}")
                print(f"    Mean jackknife ATT: {summary_stats['mean_estimate']:.3f}")
                print(f"    Std deviation: {summary_stats['std_estimate']:.3f}")
                print(f"    Range: [{summary_stats['min_estimate']:.3f}, {summary_stats['max_estimate']:.3f}]")
                print(f"    Most influential state: {summary_stats['influential_state']} (influence: {valid_results['influence'].abs().max():.3f})")
        
        return {'results': jackknife_df, 'summary': summary_stats if 'summary_stats' in locals() else {}}
    
    def specification_curve_analysis(self, 
                                   outcome: str = 'math_grade8_gap',
                ...
                try:
                    ...
                    if not result.empty and 'simple_att' in result.columns:
                        ...
                        spec_results.append({
                            ...
                            'n_treated_states': subset_data[subset_data['post_treatment']==1]['state'].nunique(),
                            'att_estimate': att_estimate,
                            'standard_error': se_estimate,
                            'ci_lower': ci_lower,
                            'ci_upper': ci_upper,
                            'significant_5pct': abs(att_estimate / se_estimate) > 1.96 if not np.isnan(se_estimate) else False
                        })
        ...
        if not spec_curve_df.empty:
            print(f"  Specification Curve Summary:")
            ...
            print(f"    Range: [{spec_curve_df['att_estimate'].min():.3f}, {spec_curve_df['att_estimate'].max():.3f}]")
            print(f"    Significant at 5%: {spec_curve_df['significant_5pct'].sum()}/{len(spec_curve_df)}")
    ...
    def placebo_tests(self, 
                     outcome: str = 'math_grade8_gap',
                     n_placebo: int = 100,
                     save_results: bool = True) -> Dict[str, Union[pd.DataFrame, dict]]:
        """
        ...
        treated_states = self.data[self.data['post_treatment'] == 1]['state'].unique()
        treatment_years = self.data[self.data['post_treatment'] == 1].groupby('state')['year'].min()
        ...
        for i in range(n_placebo):
            ...
            try:
                ...
                placebo_treated_states = np.random.choice(all_states, 
                                                        size=len(treated_states), 
                                                        replace=False)
                
                # Randomly assign treatment years
                for state in placebo_treated_states:
                    # Random treatment year (not too early or late)
                    possible_years = [y for y in available_years if y >= 2012 and y <= 2020]
                    if possible_years:
                ...
                placebo_results.append({
                    ...
                    'significant_5pct': abs(placebo_att / placebo_se) > 1.96 if not np.isnan(placebo_se) else False,
        ...
        if not placebo_df.empty:
            ...
            if not valid_placebo.empty:
                placebo_summary = {
                    ...
                    'p_value_empirical': (abs(valid_placebo['placebo_att']) >= 
                                        abs(self._get_true_estimate(outcome))).mean()
                }
                
                print(f"  Placebo Test Summary:")
                print(f"    Valid placebo tests: {placebo_summary['n_placebo']}")
                print(f"    Mean placebo ATT: {placebo_summary['mean_placebo_att']:.4f}")
                print(f"    Std deviation: {placebo_summary['std_placebo_att']:.4f}")
                print(f"    False positive rate (5%): {placebo_summary['false_positive_rate']:.3f}")
                print(f"    Empirical p-value: {placebo_summary['p_value_empirical']:.3f}")
    ...
    def create_robustness_visualization(self, 
        ...
        try:
            ...
            if not jackknife_df.empty:
                ...
                ax1.scatter(range(len(valid_jack)), valid_jack['att_estimate'], 
                           alpha=0.6, s=50)
                ax1.axhline(y=valid_jack['baseline_att'].iloc[0], color='red', 
                           linestyle='--', label='Baseline estimate')
            ...
            ax1.text(0.5, 0.5, 'Jackknife results\nnot available', 
        ...
        try:
            ...
            if not spec_df.empty:
                ...
                colors = ['red' if sig else 'blue' for sig in spec_df['significant_5pct']]
                
                ax2.scatter(range(len(spec_df)), spec_df['att_estimate'], 
                           c=colors, alpha=0.7, s=50)
                ax2.fill_between(range(len(spec_df)), 
                               spec_df['ci_lower'], spec_df['ci_upper'],
                ...
                legend_elements = [Line2D([0], [0], marker='o', color='w', 
                                        markerfacecolor='red', markersize=8, label='Significant'),
                                 Line2D([0], [0], marker='o', color='w', 
                                        markerfacecolor='blue', markersize=8, label='Not significant')]
                ax2.legend(handles=legend_elements)
        except:
            ax2.text(0.5, 0.5, 'Specification curve\nresults not available', 
        ...
        try:
            ...
            if not placebo_df.empty:
                ...
                ax3.hist(valid_placebo['placebo_att'], bins=30, alpha=0.6, 
                        color='lightblue', edgecolor='black')
                
                # Add true estimate line
                true_estimate = self._get_true_estimate(outcome)
                ax3.axvline(x=true_estimate, color='red', linestyle='--', 
                           linewidth=2, label=f'True estimate: {true_estimate:.3f}')
            ...
            ax3.text(0.5, 0.5, 'Placebo test results\nnot available', 
    ...
    def run_comprehensive_robustness(self, 
                                   outcomes: Optional[List[str]] = None) -> Dict[str, dict]:
        ...
        for outcome, results in all_results.items():
            ...
            if 'jackknife' in results and 'summary' in results['jackknife']:
                ...
                print(f"  Jackknife range: [{jack_summary.get('min_estimate', 'N/A'):.3f}, {jack_summary.get('max_estimate', 'N/A'):.3f}]")
            
            if 'specification_curve' in results and not results['specification_curve'].empty:
