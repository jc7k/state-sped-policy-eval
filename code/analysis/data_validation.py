...
class DataValidator:
    ...
    def validate_data_completeness(self) -> dict[str, any]:
        ...
        if not key_outcomes:
            ...
            key_outcomes = [
                ...
                if any(
                    x in col.lower()
                    for x in ["math", "reading", "achievement", "score"]
                )
            ]
    ...
    def generate_summary_statistics(self) -> dict[str, pd.DataFrame]:
        ...
        numeric_cols = self.master_df.select_dtypes(
            include=[np.number]
        ).columns.tolist()
    ...
    def check_balance_by_treatment(self) -> dict[str, pd.DataFrame]:
        ...
        eventually_treated = self.policy_df[self.policy_df["post_treatment"] == 1][
            "state"
        ].unique()
        ...
        balance_stats = pre_2013.groupby("eventually_treated")[balance_vars].agg(
            ["mean", "std", "count"]
        )

        # Calculate differences and t-tests
        treated_means = pre_2013[pre_2013["eventually_treated"] == True][
            balance_vars
        ].mean()
        control_means = pre_2013[pre_2013["eventually_treated"] == False][
            balance_vars
        ].mean()
        ...
        balance_results = {
            ...
            "control_states": pre_2013[~pre_2013["eventually_treated"]]["state"]
            .unique()
            .tolist(),
        }
    ...
    def analyze_outcome_trends(self) -> dict[str, any]:
        ...
        if outcome_vars and outcome_vars[0] in self.master_df.columns:
            ...
            cross_section = (
                latest_data.groupby("state")[outcome_vars[0]].mean().sort_values()
            )
        else:
        ...
        results = {
            ...
            "time_trends": trends,
            "cross_sectional_variation": cross_section.to_dict()
            if not cross_section.empty
            else {},
            "trend_analysis_year_range": (
                self.master_df["year"].min(),
                self.master_df["year"].max(),
            ),
        }
    ...
    def run_full_validation(self) -> dict[str, any]:
        ...
        self.check_balance_by_treatment()

        print("5. Analyzing outcome trends...")
        self.analyze_outcome_trends()
    ...
    def generate_validation_report(
        self, output_path: str = "data/reports/validation_report.txt"
    ):
        ...
        with open(output_path, "w") as f:
            ...
            if "completeness" in self.validation_results:
                ...
                f.write(
                    f"Completely Missing Variables: {len(comp['completely_missing_vars'])}\n\n"
                )
            ...
            if "outcomes" in self.validation_results:
                ...
                f.write(
                    f"Outcome Variables Found: {len(outcomes['outcome_variables'])}\n"
                )
...
if __name__ == "__main__":
    ...
    if "completeness" in results:
        ...
        print(
            f"Dataset: {comp['total_observations']} obs, {comp['total_variables']} vars"
        )
    ...
    if "balance" in results:
        ...
        print(
            f"Treatment: {len(balance['treated_states'])} treated, {len(balance['control_states'])} control states"
        )
