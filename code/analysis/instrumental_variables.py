...
class InstrumentalVariablesAnalyzer:
    ...
    def __init__(
        ...
        print("InstrumentalVariablesAnalyzer initialized:")
        ...
        if save_results:
            ...
            validation_df = pd.DataFrame(
                [
                    {
                        "outcome": outcome,
                        "first_stage_f_stat": first_stage_results.get(
                            "f_statistic", np.nan
                        ),
                        "first_stage_p_value": first_stage_results.get(
                            "f_pvalue", np.nan
                        ),
                        "weak_instruments": first_stage_results.get("f_statistic", 0)
                        < 10,
                        "exclusion_court_corr": exclusion_results.get(
                            "court_outcome_corr", np.nan
                        ),
                        "exclusion_monitoring_corr": exclusion_results.get(
                            "monitoring_outcome_corr", np.nan
                        ),
                        "balance_court_diff": balance_results.get(
                            "court_balance_diff", np.nan
                        ),
                        "balance_monitoring_diff": balance_results.get(
                            "monitoring_balance_diff", np.nan
                        ),
                    }
                ]
            )
    ...
    def _prepare_iv_data(self, outcome: str) -> pd.DataFrame:
        ...
        print(
            f"  Missing data reduced sample from {len(self.data)} to {len(analysis_data)}"
        )
    ...
    def _test_exclusion_restriction(
        self, data: pd.DataFrame, outcome: str
    ) -> dict[str, float]:
    ...
    def estimate_iv_effects(
        ...
        try:
            ...
            exogenous = analysis_data[exogenous_vars] if exogenous_vars else None
            ...
            results = {
                ...
                "first_stage_f": getattr(iv_results, "first_stage", {}).get(
                    "f_statistic", np.nan
                ),
                "overid_pvalue": getattr(iv_results, "j_statistic", {}).get(
                    "pvalue", np.nan
                )
                if hasattr(iv_results, "j_statistic")
                else np.nan,
            }
    ...
    def compare_ols_iv_estimates(
        ...
        try:
            ...
            if "log_total_revenue_per_pupil" in analysis_data.columns:
                X = pd.concat(
                    [X, analysis_data[["log_total_revenue_per_pupil"]]], axis=1
                )
            ...
            if iv_results:
                ...
                se_diff = (
                    np.sqrt(iv_se**2 - ols_se**2)
                    if iv_se**2 > ols_se**2
                    else np.sqrt(iv_se**2 + ols_se**2)
                )
                hausman_stat = (coeff_diff / se_diff) ** 2 if se_diff > 0 else np.nan
                hausman_pvalue = (
                    1 - stats.chi2.cdf(hausman_stat, 1)
                ...
                comparison_results["hausman"] = {
                    ...
                    "endogeneity_detected": hausman_pvalue < 0.05
                ...
                print(
                    f"    Endogeneity detected: {'Yes' if hausman_pvalue < 0.05 else 'No'}"
                )
                    ...
                    [
                        {
                            ...
                            "iv_coefficient": comparison_results["iv"]["iv_coefficient"]
                            if "iv" in comparison_results
                            else np.nan,
                            "iv_std_error": comparison_results["iv"]["iv_std_error"]
                            if "iv" in comparison_results
                            else np.nan,
                            "iv_pvalue": comparison_results["iv"]["iv_pvalue"]
                            if "iv" in comparison_results
                            else np.nan,
                            "coefficient_difference": comparison_results["hausman"][
                                "coefficient_difference"
                            ]
                            if "hausman" in comparison_results
                            else np.nan,
                            "hausman_pvalue": comparison_results["hausman"]["pvalue"]
                            if "hausman" in comparison_results
                            else np.nan,
                            "endogeneity_detected": comparison_results["hausman"][
    ...
    def create_iv_summary_visualization(self, save_formats: list[str] = None) -> str:
        ...
        try:
            ...
            if ols_iv_results:
                ...
                ax1.bar(
                    x - width / 2,
                    results_df["ols_coeff"],
                    width,
                    yerr=results_df["ols_se"],
                    label="OLS",
                    alpha=0.7,
                )
                ax1.bar(
            ...
            ax1.text(
                0.5,
                0.5,
                f"OLS vs IV comparison error: {str(e)[:30]}...",
                ha="center",
                va="center",
        ...
        try:
            ...
            if not all(np.isnan(f_stats)):
                ...
                ax2.bar(outcomes_clean, f_stats, alpha=0.7)
                ax2.axhline(
                    y=10,
                    color="red",
                    linestyle="--",
        ...
        try:
            if not self.data.empty:
                ...
                if instrument_counts:
                    ...
                    values = [
                        instrument_counts.get("court_ordered", 0),
                        instrument_counts.get("under_monitoring", 0),
                    ]

                    ax3.bar(
                        labels, values, alpha=0.7, color=["steelblue", "darkorange"]
                    )
        ...
        try:
            if not self.data.empty:
                ...
                n_treated_states = self.data[self.data["post_treatment"] == 1][
                    "state"
                ].nunique()
                ...
                summary_text += "Instrument Validity:\\n"
                summary_text += "  ✓ Relevance: Court orders/monitoring\\n"
                summary_text += "    predict policy reform adoption\\n"
                summary_text += "  ✓ Exclusion: Instruments affect outcomes\\n"
                summary_text += "    only through policy changes\\n"
                summary_text += "  ✓ Exogeneity: External legal/federal\\n"
                summary_text += "    processes drive instrument variation\\n\\n"

                summary_text += "Key Advantages:\\n"
                summary_text += "  • Addresses endogeneity concerns\\n"
                summary_text += "  • Plausibly exogenous variation\\n"
                summary_text += "  • Multiple instruments for testing"
        except:
    ...
    def run_complete_iv_analysis(self) -> dict[str, any]:
        ...
        for outcome, results in all_results.items():
            ...
            if "validation" in results and "first_stage" in results["validation"]:
                ...
                print(
                    f"  First-stage F-stat: {f_stat:.1f} {'(weak)' if weak_instruments else '(strong)'}"
                )

            if (
                "ols_iv_comparison" in results
                and "hausman" in results["ols_iv_comparison"]
            ):
                endogeneity = results["ols_iv_comparison"]["hausman"].get(
