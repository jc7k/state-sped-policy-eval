...
class COVIDAnalyzer:
    ...
    def __init__(self, 
        ...
        print(f"COVIDAnalyzer initialized:")
    ...
    def _analyze_covid_data_availability(self):
        ...
        for outcome in self.outcomes:
            ...
            print(f"  {outcome}: COVID={covid_available}, Pre-COVID={pre_covid_available}")
    ...
    def prepare_triple_diff_data(self, outcome: str) -> pd.DataFrame:
        ...
        analysis_data['covid_period'] = (analysis_data['year'].isin(self.covid_years)).astype(int)
    ...
    def estimate_triple_difference(self, 
        ...
        try:
            ...
            # Estimate model
            model = ols(formula, data=analysis_data).fit(cov_type='cluster', 
                                                       cov_kwds={'groups': analysis_data['state']})
            ...
            print(f"  Triple-difference coefficient: {ddd_coeff:.3f} (SE: {ddd_se:.3f})")
            ...
            # Interpretation
            if ddd_coeff > 0:
                print(f"  → Policy reforms WORSENED outcomes during COVID (less resilient)")
            else:
                print(f"  → Policy reforms IMPROVED resilience during COVID")
            ...
            if save_results:
                ...
                # Save model summary
                summary_path = self.results_dir / f"covid_ddd_model_summary_{outcome}.txt"
    ...
    def analyze_resilience_by_reform_timing(self, 
                                          outcome: str = 'math_grade8_gap',
        ...
        analysis_data['first_reform_year'] = analysis_data['first_reform_year'].fillna(9999)  # Never treated
        
        # Create reform timing categories
        analysis_data['reform_timing'] = 'Never Reformed'
        analysis_data.loc[analysis_data['first_reform_year'] <= 2017, 'reform_timing'] = 'Early Reformer (≤2017)'
        analysis_data.loc[(analysis_data['first_reform_year'] >= 2018) & 
                         (analysis_data['first_reform_year'] <= 2020), 'reform_timing'] = 'Mid Reformer (2018-2020)'
        analysis_data.loc[analysis_data['first_reform_year'] >= 2021, 'reform_timing'] = 'Late Reformer (≥2021)'
        ...
        for timing_group in analysis_data['reform_timing'].unique():
            ...
            group_data = analysis_data[analysis_data['reform_timing'].isin([timing_group, 'Never Reformed'])].copy()
            
            if len(group_data) < 20:
                continue
            
            # Create binary indicator for this timing group
            group_data['timing_group'] = (group_data['reform_timing'] == timing_group).astype(int)
            ...
            try:
                ...
                print(f"  {timing_group}: Resilience = {covid_resilience:.3f} (p={covid_resilience_p:.3f})")
    ...
    def create_covid_analysis_visualization(self, 
        ...
        try:
            ...
            if ddd_results:
                ...
                bars = ax1.bar(outcomes_clean, results_df['ddd_coeff'],
                              yerr=results_df['ddd_se'], color=colors, alpha=0.7, capsize=5)
                
                ax1.axhline(y=0, color='black', linestyle='-', alpha=0.8)
                ax1.set_title('COVID Resilience Effects by Outcome\n(Triple-Difference Coefficients)', fontweight='bold')
                ...
                legend_elements = [Line2D([0], [0], color='red', lw=4, alpha=0.7, label='Significant'),
                                 Line2D([0], [0], color='steelblue', lw=4, alpha=0.7, label='Not significant')]
                ax1.legend(handles=legend_elements)
        except Exception as e:
            ax1.text(0.5, 0.5, f'DDD coefficients error', 
        ...
        try:
            if not self.data.empty:
                ...
                for year in range(2017, 2023):
                    ...
                    available_outcomes = sum(year_data[outcome].notna().sum() for outcome in self.outcomes)
                    year_counts.append({'year': year, 'available_outcomes': available_outcomes})
                
                year_df = pd.DataFrame(year_counts)
                
                colors = ['steelblue' if year < 2020 else 'orange' for year in year_df['year']]
                bars = ax2.bar(year_df['year'], year_df['available_outcomes'], 
                              color=colors, alpha=0.7)
                
                ax2.axvline(x=2019.5, color='red', linestyle='--', alpha=0.7, linewidth=2, label='COVID Onset')
            ...
            ax2.text(0.5, 0.5, 'Data availability error', 
        ...
        try:
            if not self.data.empty:
                ...
                colors = ['darkblue', 'steelblue', 'orange', 'red'][:len(timing_counts)]
                ax3.pie(timing_counts.values, labels=timing_counts.index, 
                       colors=colors, autopct='%1.0f%%')
                ax3.set_title('Reform Timing Relative to COVID', fontweight='bold')
        except Exception as e:
            ax3.text(0.5, 0.5, 'Reform timing error', 
        ...
        try:
            if not self.data.empty:
                ...
                summary_text += f"Natural Experiment Design:\\n"
                ...
                summary_text += f"  DDD = (Reform vs Control) ×\\n"
                summary_text += f"        (COVID vs Pre-COVID) ×\\n"
                summary_text += f"        (SWD vs Non-SWD gap)\\n\\n"
                
                summary_text += f"Key Research Questions:\\n"
                summary_text += f"  • Which reforms provided COVID resilience?\\n"
                summary_text += f"  • Did early reformers fare better?\\n"
                summary_text += f"  • What policies protected SWD outcomes?\\n\\n"
                
                summary_text += f"Policy Implications:\\n"
                summary_text += f"  • Crisis-resilient policy design\\n"
                summary_text += f"  • Emergency response effectiveness\\n"
                summary_text += f"  • Future pandemic preparedness"
        except:
    ...
    def run_complete_covid_analysis(self) -> Dict[str, any]:
        ...
        for outcome, results in all_results.items():
            ...
            if 'triple_difference' in results and results['triple_difference']:
                ...
                significant = results['triple_difference'].get('significant_5pct', False)
                
                print(f"  COVID resilience effect: {ddd_coeff:.3f} (p={ddd_pvalue:.3f})")
                print(f"  Significant: {'Yes' if significant else 'No'}")
                print(f"  Interpretation: {'More resilient' if ddd_coeff < 0 else 'Less resilient'}")
        ...
        print(f"  Outcomes with significant COVID effects: {significant_effects}/{len(self.outcomes)}")
        print(f"  Outcomes showing policy resilience: {resilient_policies}/{len(self.outcomes)}")
