...
class SimpleRobustnessChecker:
    ...
    def __init__(
        ...
        print(
            f"  Data: {self.analyzer.df.shape if hasattr(self.analyzer, 'df') else 'N/A'}"
        )
    ...
    def analyze_treatment_balance(self) -> pd.DataFrame:
        ...
        for var in key_vars:
            if var in pre_treatment.columns:
                treated_mean = pre_treatment[
                    pre_treatment["state"].isin(treated_states)
                ][var].mean()
                control_mean = pre_treatment[
                    pre_treatment["state"].isin(control_states)
                ][var].mean()
        ...
        for outcome in self.outcomes:
            try:
                ...
                if not result.empty and "simple_att" in result.columns:
                    ...
                    effect_size = (
                        att / outcome_data.std() if not outcome_data.empty else np.nan
                    )
                    ...
                    effect_results.append(
                        {
                            ...
                            "significant_5pct": abs(att / se) > 1.96
                            if not np.isnan(se)
                            else False,
    ...
    def create_robustness_summary_plot(self, save_formats: list[str] = None) -> str:
        ...
        try:
            ...
            if effects_data:
                ...
                outcomes_clean = [
                    self._format_outcome_label(o) for o in effects_df["outcome"]
                ]
        ...
        try:
            if hasattr(self.analyzer, "df") and self.analyzer.df is not None:
                ...
                summary_text += (
                    f"  Control states: {df['state'].nunique() - treated_states}\\n\\n"
                )
    ...
    def run_complete_robustness_check(self) -> dict[str, any]:
        ...
        if (
            "consistency_analysis" in results
            and not results["consistency_analysis"].empty
        ):
