...
class PanelDataProcessor:
    ...
    def create_naep_outcomes(self) -> pd.DataFrame:
        ...
        if Path(naep_path).exists():
            ...
            naep_df = naep_pivot.rename(
                columns={"SWD": "swd_score", "non-SWD": "non_swd_score"}
            )
            ...
            for subject in ["mathematics", "reading"]:
                for grade in [4, 8]:
                    subset = naep_df[
                        (naep_df["subject"] == subject) & (naep_df["grade"] == grade)
                    ]
                    if not subset.empty:
    ...
    def create_finance_variables(self) -> pd.DataFrame:
        ...
        if Path(finance_path).exists():
            ...
            existing_vars = [
                var for var in finance_vars if var in finance_outcomes.columns
            ]

            if existing_vars:
                # Select relevant columns
                finance_subset = finance_outcomes[
                    ["state", "year"] + existing_vars
                ].copy()
    ...
    def create_edfacts_variables(self) -> pd.DataFrame:
        ...
        edfacts_cols = [
            col for col in self.master_df.columns if "edfacts" in col.lower()
        ]
        if edfacts_cols:
    ...
    def merge_all_data(self) -> pd.DataFrame:
        ...
        if not naep_outcomes.empty:
            analysis_df = analysis_df.merge(
                naep_outcomes, on=["state", "year"], how="left"
            )
        ...
        if not finance_vars.empty:
            analysis_df = analysis_df.merge(
                finance_vars, on=["state", "year"], how="left"
            )
        ...
        if not edfacts_vars.empty:
            analysis_df = analysis_df.merge(
                edfacts_vars, on=["state", "year"], how="left"
            )
    ...
    def create_analysis_panel(
        self, output_path: str = "data/final/analysis_panel.csv"
    ) -> str:
        ...
        print("\nPanel Summary:")
        print(
            f"Years: {self.analysis_df['year'].min()} - {self.analysis_df['year'].max()}"
        )
        ...
        outcome_cols = [
            col for col in self.analysis_df.columns if "gap" in col or "score" in col
        ]
        if outcome_cols:
    ...
    def generate_summary_stats(self) -> dict[str, pd.DataFrame]:
        ...
        if self.analysis_df is None:
            raise ValueError(
                "Analysis panel not created yet - run create_analysis_panel first"
            )
...
if __name__ == "__main__":
    ...
    if processor.analysis_df is not None:
        ...
        available_vars = [
            var for var in key_vars if var in summaries["overall"].columns
        ]
        if available_vars:
