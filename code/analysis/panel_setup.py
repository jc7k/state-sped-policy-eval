...
class PanelDataProcessor:
    ...
    def create_naep_outcomes(self) -> pd.DataFrame:
        ...
        if Path(naep_path).exists():
            ...
            naep_df = naep_pivot.rename(columns={
                'SWD': 'swd_score',
                'non-SWD': 'non_swd_score'
            })
            ...
            for subject in ['mathematics', 'reading']:
                for grade in [4, 8]:
                    ...
                    if not subset.empty:
                        ...
                        subset_renamed = subset[['state', 'year', 'swd_score', 'non_swd_score', 'achievement_gap']].copy()
                        subset_renamed.columns = ['state', 'year', 
                        ...
                        if outcome_vars is None:
                            ...
                            outcome_vars = outcome_vars.merge(subset_renamed, on=['state', 'year'], how='outer')
    ...
    def create_finance_variables(self) -> pd.DataFrame:
        ...
        if Path(finance_path).exists():
            ...
            finance_vars = ['total_revenue', 'federal_revenue', 'state_revenue', 'local_revenue',
            ...
            if existing_vars:
                ...
                for var in existing_vars:
                    finance_subset[f'{var}_per_pupil'] = finance_subset[var] / 1000  # Convert to thousands
                
                return finance_subset
        
        # Fallback to master dataset finance columns
        finance_cols = [col for col in self.master_df.columns 
    ...
    def _create_derived_variables(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for lead in range(1, 6):  # 5 years of leads
            df[f'lead_{lead}'] = np.where(
                df['years_since_treatment'] == -lead,
                1, 0
            )
        
        # Lag treatment indicators  
        for lag in range(1, 6):  # 5 years of lags
            df[f'lag_{lag}'] = np.where(
                df['years_since_treatment'] == lag,
                1, 0
            )
    ...
    def create_analysis_panel(self, output_path: str = "data/final/analysis_panel.csv") -> str:
        ...
        print(f"Treated states: {len(self.analysis_df[self.analysis_df['post_treatment']==1]['state'].unique())}")
        ...
        treatment_cols = [col for col in self.analysis_df.columns if any(x in col for x in ['treated', 'post_treatment', 'monitoring'])]
        if treatment_cols:
    ...
    def generate_summary_stats(self) -> Dict[str, pd.DataFrame]:
        ...
        # By treatment status
        treated_summary = self.analysis_df[self.analysis_df['post_treatment']==1][numeric_cols].describe()
        control_summary = self.analysis_df[self.analysis_df['post_treatment']==0][numeric_cols].describe()
...
if __name__ == "__main__":
    ...
    if processor.analysis_df is not None:
        ...
        key_vars = ['year', 'post_treatment', 'years_since_treatment', 'under_monitoring']
