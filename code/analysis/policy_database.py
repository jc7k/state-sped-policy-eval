...
class PolicyDatabase:
    ...
    def get_treatment_timing(
        self, treatment_type: str = "funding_formula"
    ) -> pd.DataFrame:
        """
        ...
        if treatment_type == "funding_formula":
            reforms = self.reforms_df[
                self.reforms_df["reform_type"] == "funding_formula"
            ].copy()
    ...
    def _create_treatment_panel(
        self, events_df: pd.DataFrame, year_col: str
    ) -> pd.DataFrame:
    ...
    def _create_monitoring_panel(self) -> pd.DataFrame:
        ...
        for state in states:
            for year in years:
                ...
                if not state_events.empty:
                    starts = state_events[
                        state_events["event_type"] == "monitoring_start"
                    ]["year"].tolist()
                    ends = state_events[state_events["event_type"] == "monitoring_end"][
                        "year"
                    ].tolist()
                ...
                panel_data.append(
                    {"state": state, "year": year, "under_monitoring": under_monitoring}
                )
    ...
    def get_all_treatments(self) -> pd.DataFrame:
        ...
        court_treated = court_panel[court_panel["post_treatment"] == 1][
            ["state", "year"]
        ].copy()
    ...
    def _add_state_characteristics(self, panel_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in ["region", "political_lean", "unionized"]:
            panel_df[col] = panel_df["state"].map(
                lambda x, col=col: state_chars.get(x, {}).get(col, "Unknown")
            )
    ...
    def validate_treatment_timing(self) -> dict[str, any]:
        ...
        validation_results = {
            ...
            "states_with_funding_reforms": len(
                full_panel[full_panel["post_treatment"] == 1]["state"].unique()
            ),
            "years_with_reforms": sorted(
                self.reforms_df["reform_year"].unique().tolist()
            ),
            "states_under_monitoring": len(
                full_panel[full_panel["under_monitoring"] == 1]["state"].unique()
            ),
            "court_ordered_states": len(
                full_panel[full_panel["court_ordered"] == 1]["state"].unique()
            ),
            "treatment_balance": full_panel.groupby("year")["post_treatment"]
            .mean()
            .to_dict(),
        }
