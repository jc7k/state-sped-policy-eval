...
class PublicationGenerator:
    ...
    def __init__(self, project_root: Path | None = None):
        ...
        logger.info(
            f"PublicationGenerator initialized: Project root: {self.project_root}"
        )
    ...
    def load_did_results(self) -> dict[str, pd.DataFrame]:
        ...
        for outcome in self.outcomes:
            ...
            if file_path.exists():
                ...
                logger.info(
                    f"Loaded DiD aggregated effects for {outcome}: {len(df)} rows"
                )
            else:
    ...
    def load_covid_results(self) -> dict[str, pd.DataFrame]:
        ...
        logger.info(
            f"Loaded COVID results for {len(covid_results)} outcome-analysis combinations"
        )
    ...
    def create_main_results_table(self) -> pd.DataFrame:
        ...
        for outcome in self.outcomes:
            ...
            if not iv_results.empty and outcome in iv_results["outcome"].values:
                ...
                row["IV_Effect"] = iv_row.get(
                    "iv_coefficient", iv_row.get("iv_effect", np.nan)
                )
                ...
                if (
                    pd.notna(row["IV_Effect"])
                    and pd.notna(row["IV_SE"])
                    and row["IV_SE"] > 0
                ):
                    ...
                    row["IV_P"] = 2 * (
                        1 - t.cdf(abs(t_stat), df=148)
                    )  # 150 obs - 2 parameters
    ...
    def create_summary_statistics_table(self) -> pd.DataFrame:
        ...
        for var in key_vars:
            if var in df.columns:
                ...
                stats = {
                    "Variable": self.outcome_labels.get(
...
"""
        ...
        for _, row in main_results.iterrows():
            ...
            significance = (
                " *"
                if pd.notna(did_p) and str(did_p) != "" and float(did_p) < 0.05
                else ""
            )
