...
class PublicationGenerator:
    ...
    def load_iv_results(self) -> pd.DataFrame:
        ...
        if iv_file.exists():
            ...
            if iv_file.exists():
                ...
                logger.warning(f"IV results file not found")
                return pd.DataFrame()
    ...
    def create_main_results_table(self) -> pd.DataFrame:
        ...
        for outcome in self.outcomes:
            ...
            if outcome in did_results and not did_results[outcome].empty:
                ...
                if not did_df.empty:
                    ...
                    row['DiD_Effect'] = agg_row.get('simple_att', agg_row.get('weighted_att', agg_row.get('coefficient', np.nan)))
                    row['DiD_SE'] = agg_row.get('simple_se', agg_row.get('weighted_se', agg_row.get('se', np.nan)))
        ...
        for col in ['DiD_Effect', 'DiD_SE', 'IV_Effect', 'IV_SE', 'COVID_Effect', 'COVID_SE']:
            if col in results_df.columns:
                results_df[col] = results_df[col].round(3)
                
        for col in ['DiD_P', 'IV_P', 'COVID_P']:
            if col in results_df.columns:
                results_df[col] = results_df[col].apply(lambda x: f"{x:.3f}" if pd.notna(x) and str(x) != "" else "")
...
"""
        ...
        for _, row in main_results.iterrows():
            ...
            did_p = row['DiD_P']
            significance = " *" if pd.notna(did_p) and str(did_p) != "" and float(did_p) < 0.05 else ""
...
"""
    ...
    def save_publication_outputs(self) -> Dict[str, Path]:
        ...
        summary_info = {
            ...
            'analysis_methods': ['Staggered DiD', 'Instrumental Variables', 'COVID Triple-Difference'],
