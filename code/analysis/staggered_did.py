...
class StaggeredDiDAnalyzer:
    ...
    def estimate_group_time_effects(self, 
                                  outcome_var: str,
        ...
        if outcome_var not in self.df.columns:
            ...
            raise ValueError(f"Outcome variable '{outcome_var}' not found. Available: {available_outcomes}")
        ...
        for treatment_year, treated_states in cohorts.items():
            ...
            for t in post_periods:
                ...
                analysis_data['treated_group'] = analysis_data['state'].isin(treated_states)
                analysis_data['post_period'] = (analysis_data['year'] == t).astype(int)
                analysis_data['treat_post'] = analysis_data['treated_group'] * analysis_data['post_period']
                ...
                try:
                    ...
                    formula_parts = [outcome_var, 'treated_group + post_period + treat_post']
                    ...
                    result = {
                        ...
                        'n_treated': len(analysis_data[analysis_data['treated_group']==True]),
                        'n_control': len(analysis_data[analysis_data['treated_group']==False])
                    }
                    
                    group_time_results.append(result)
                    
                except Exception as e:
                    print(f"Error estimating for cohort {treatment_year}, time {t}: {e}")
                    continue
    ...
    def aggregate_treatment_effects(self, group_time_df: pd.DataFrame) -> Dict[str, float]:
        ...
        relative_time_effects = group_time_df.groupby('time_relative').agg({
            'att': 'mean',
    ...
    def estimate_event_study(self, 
        ...
        if not event_time_vars:
            ...
            for lead in range(1, max_lead + 1):
                analysis_data[f'lead_{lead}'] = (analysis_data['years_since_treatment'] == -lead).astype(int)
                event_time_vars.append(f'lead_{lead}')
            
            for lag in range(2, max_lag + 1):
                analysis_data[f'lag_{lag}'] = (analysis_data['years_since_treatment'] == lag).astype(int)
        ...
        try:
            ...
            model = ols(formula, data=analysis_data).fit(cov_type='cluster', cov_kwds={'groups': analysis_data['state']})
            ...
            print(f"Event study estimated successfully:")
    ...
    def run_staggered_analysis(self, outcome_vars: Optional[List[str]] = None) -> Dict[str, any]:
        """
        ...
        for outcome in outcome_vars:
            ...
            try:
                ...
                if aggregated:
                    ...
                    print(f"  Average Treatment Effect: {aggregated['simple_att']:.3f} ({aggregated['simple_se']:.3f})")
                    print(f"  Weighted Treatment Effect: {aggregated['weighted_att']:.3f} ({aggregated['weighted_se']:.3f})")
        ...
        if 'staggered_analysis' in self.results:
            for outcome, results in self.results['staggered_analysis'].items():
                ...
                if results['aggregated_effects']:
                    agg_data = {k: [v] for k, v in results['aggregated_effects'].items() 
