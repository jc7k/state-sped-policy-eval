...
class DataIntegrator:
    ...
    def load_naep_data(self) -> pd.DataFrame:
        ...
        key_vars = ['state', 'year', 'subject', 'grade', 'avg_scale_score', 
                   'pct_below_basic', 'pct_basic', 'pct_proficient', 'pct_advanced']
        
        available_vars = [var for var in key_vars if var in df.columns]
        df = df[available_vars].copy()
        
        self.logger.info(f"Loaded NAEP data: {len(df)} records, {len(df['state'].unique())} states")
        return df
    ...
    def load_census_data(self) -> pd.DataFrame:
        ...
        financial_vars = ['total_revenue', 'total_expenditure', 'current_expenditure', 
                         'instruction_expenditure', 'per_pupil_expenditure', 'special_education_expenditure']
        
        for var in financial_vars:
            if var in df.columns:
                df[var] = pd.to_numeric(df[var], errors='coerce')
        
        self.logger.info(f"Loaded Census data: {len(df)} records, {len(df['state'].unique())} states")
        return df
    ...
    def load_edfacts_data(self) -> pd.DataFrame:
            ...
            for pattern in patterns:
                ...
                if file_path.exists():
                    ...
                    try:
                        ...
                        if 'YEAR' not in df_year.columns and 'Year' not in df_year.columns:
                            df_year['year'] = year
        ...
        if 'state' in df.columns:
            ...
            valid_state_names = list(self.state_mapping['state_name'].keys()) + ['District of Columbia']
        ...
        self.logger.info(f"Loaded EdFacts data: {len(df)} records, {len(df['state'].unique())} states")
        return df
    ...
    def load_ocr_data(self) -> pd.DataFrame:
        ...
        for filename, year in csv_files:
            ...
            if file_path.exists():
                try:
                    ...
                    self.logger.info(f"Loaded OCR CSV: {filename} ({len(df_year)} records)")
                except Exception as e:
                ...
                for discipline_file in discipline_files:
                    ...
                    if found_files:
                        ...
                        try:
                            ...
                            df_disc['year'] = year
                            df_disc['data_type'] = discipline_file.replace('.csv', '').lower()
                            all_data.append(df_disc)
                            self.logger.info(f"Loaded OCR {year} {discipline_file}: {len(df_disc)} records")
                        except Exception as e:
                            self.logger.warning(f"Error reading {file_path}: {e}")
        ...
        # Convert numeric columns
        numeric_cols = ['year', 'total_enrollment', 'swd_enrollment', 'suspensions', 
                       'expulsions', 'restraint_incidents', 'seclusion_incidents']
        for col in numeric_cols:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors='coerce')
        
        self.logger.info(f"Loaded OCR data: {len(df)} records, {len(df['state'].unique())} states")
        return df
    
    def create_state_year_panel(self, datasets: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        """
        ...
        self.logger.info(f"Created base panel: {len(master_df)} state-year observations")
        ...
        for dataset_name, df in datasets.items():
            ...
            if dataset_name == 'naep':
                ...
                if len(numeric_cols) > 0 and 'state' in df.columns and 'year' in df.columns:
                    ...
                    naep_cols = {col: f'naep_{col}' for col in naep_agg.columns if col not in ['state', 'year']}
                    naep_agg = naep_agg.rename(columns=naep_cols)
                    
                    master_df = master_df.merge(naep_agg, on=['state', 'year'], how='left')
                    self.logger.info(f"Merged NAEP data: {len(naep_agg)} state-year observations")
            
            elif dataset_name == 'census':
                # Census: Should be one observation per state-year
                census_vars = [col for col in df.columns if col not in ['state', 'year']]
                if census_vars:
                    census_subset = df[['state', 'year'] + census_vars].copy()
                    master_df = master_df.merge(census_subset, on=['state', 'year'], how='left')
                ...
                if 'state' in df.columns and 'year' in df.columns:
                    ...
                    if count_cols:
                        ...
                        edfacts_agg = df.groupby(['state', 'year']).agg(agg_dict).reset_index()
                        
                        # Add EdFacts prefix
                        edfacts_cols = {col: f'edfacts_{col}' for col in edfacts_agg.columns if col not in ['state', 'year']}
                        edfacts_agg = edfacts_agg.rename(columns=edfacts_cols)
                        
                        master_df = master_df.merge(edfacts_agg, on=['state', 'year'], how='left')
                        self.logger.info(f"Merged EdFacts data: {len(edfacts_agg)} state-year observations")
                ...
                if 'state' in df.columns and 'year' in df.columns:
                    ...
                    if count_cols:
                        ...
                        agg_dict = {col: 'sum' for col in count_cols}
                        ocr_agg = df.groupby(['state', 'year']).agg(agg_dict).reset_index()
                        
                        # Add OCR prefix
                        ocr_cols = {col: f'ocr_{col}' for col in ocr_agg.columns if col not in ['state', 'year']}
                        ocr_agg = ocr_agg.rename(columns=ocr_cols)
                        
                        master_df = master_df.merge(ocr_agg, on=['state', 'year'], how='left')
                        self.logger.info(f"Merged OCR data: {len(ocr_agg)} state-year observations")
        
        self.logger.info(f"Master dataset created: {len(master_df)} observations, {len(master_df.columns)} variables")
        return master_df
    ...
    def add_derived_variables(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'naep_pct_proficient' in df.columns:
            df['naep_proficiency_rate'] = df['naep_pct_proficient'] + df.get('naep_pct_advanced', 0)
        
        # Per-pupil metrics
        if 'total_expenditure' in df.columns and 'ocr_total_enrollment' in df.columns:
            df['per_pupil_total_expenditure'] = df['total_expenditure'] / df['ocr_total_enrollment']
        
        # Special education rates
        if 'edfacts_total_swd' in df.columns and 'ocr_total_enrollment' in df.columns:
            df['swd_identification_rate'] = df['edfacts_total_swd'] / df['ocr_total_enrollment']
        ...
        self.logger.info(f"Added derived variables. Final dataset: {len(df.columns)} variables")
        return df
    ...
    def run_integration(self) -> pd.DataFrame:
        ...
        self.logger.info(f"Integration complete. Output files: {list(output_files.keys())}")
...
def main():
    ...
    parser.add_argument('--raw-data-dir', type=Path, default='data/raw',
    ...
    print(f"Non-missing observations by key variables:")
    
    key_vars = ['naep_avg_scale_score', 'total_expenditure', 'edfacts_total_swd', 'ocr_total_enrollment']
    for var in key_vars:
