...
from .utils import get_optimal_figure_size
...
class TreatmentEffectsDashboard(BaseVisualizer):
    ...
    def _load_available_data(self) -> None:
        ...
        self.treatment_effects = self._load_csv_file(
            "aggregated_effects_*.csv", cache_key="aggregated_effects"
        )
    ...
    def create_treatment_timeline_map(
        ...
        first_treatment = (
            treated_states.groupby("state", sort=False)["year"]
            .min()
            .reset_index(name="reform_year")
        )
    ...
    def create_visualization(
        self, plot_type: str = "timeline_map", outcome: str = "", **kwargs
    ) -> str:
        """
...
if __name__ == "__main__":
    ...
    print(
        f"Cache utilization: {cache_info['utilization_percent']:.1f}% "
