...
class TestDataProcessingPerformance:
    ...
    def test_merge_performance_multiple_sources(self):
        ...
        base_data = pd.DataFrame({"state": ["AL", "CA", "TX"] * 10000, "year": [2022] * 30000})
...
class TestEndToEndPerformance:
        ...
        with patch("src.collection.naep_collector.requests.get") as mock_get:
            ...
            with patch("src.collection.naep_collector.time.sleep"):
                for name, collector in collectors.items():
                    ...
                    df = collector.fetch_state_swd_data(
                        [2019, 2020, 2021, 2022], [4, 8], ["mathematics", "reading"]
                    )
    ...
    def test_concurrent_collection_performance(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            futures = [
                executor.submit(mock_collection_task, year) for year in [2019, 2020, 2021, 2022]
            ]
...
class TestResourceMonitoring:
    ...
    def test_disk_io_monitoring(self, temp_data_dir):
        ...
        for i in range(10):
            df = pd.DataFrame({"state": ["AL"] * 1000, "year": [2022] * 1000, "data": range(1000)})
