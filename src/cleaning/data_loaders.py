...
from .base_loader import DirectoryBasedLoader, FileBasedLoader
from .utils import ColumnStandardizer, DataCleaningUtils
...
class CensusDataLoader(FileBasedLoader):
    ...
    def __init__(self, raw_data_dir: Path):
        file_patterns = ["census_education_finance_parsed.csv", "census_f33_*.csv", "census_*.csv"]
        super().__init__(raw_data_dir, file_patterns)
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        existing_financial_vars = [var for var in financial_vars if var in df_clean.columns]

        if existing_financial_vars:
            df_clean = DataCleaningUtils.safe_numeric_conversion(df_clean, existing_financial_vars)
...
class EdFactsDataLoader(DirectoryBasedLoader):
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_clean = DataCleaningUtils.standardize_state_codes(df_clean, "state", "state_name")
