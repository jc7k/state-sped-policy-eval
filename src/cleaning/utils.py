...
class DataCleaningUtils:
    ...
    def standardize_state_codes(
        df: pd.DataFrame, state_column: str = "state", state_name_column: str | None = None
    ) -> pd.DataFrame:
        """
        ...
        if state_name_column and state_name_column in df_clean.columns:
            df_clean[state_column] = df_clean[state_name_column].apply(StateUtils.name_to_abbrev)
    ...
    def validate_required_columns(
        df: pd.DataFrame, required_columns: list[str], dataset_name: str = "dataset"
    ) -> None:
        """
        ...
        if missing_columns:
            raise CleaningError(f"{dataset_name} missing required columns: {missing_columns}")
    ...
    def aggregate_by_groups(
        df: pd.DataFrame, group_columns: list[str], agg_columns: dict[str, str], prefix: str = ""
    ) -> pd.DataFrame:
        """
        ...
        if prefix:
            rename_dict = {
                col: f"{prefix}_{col}" for col in aggregated.columns if col not in group_columns
            }
...
class ColumnStandardizer:
    ...
    def standardize_columns(
        df: pd.DataFrame, custom_mappings: dict[str, str] | None = None
    ) -> pd.DataFrame:
        """
