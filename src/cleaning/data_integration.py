...
class DataIntegrator(ErrorHandlingMixin):
    ...
    def __init__(
        ...
        self.log_info("Initialized DataIntegrator with:")
    ...
    def load_datasets(self, sources: list | None = None) -> dict[str, pd.DataFrame]:
        """
    ...
    def save_individual_datasets(self, datasets: dict[str, pd.DataFrame]) -> dict[str, Path]:
        """
    ...
    def create_master_dataset(self, datasets: dict[str, pd.DataFrame]) -> pd.DataFrame:
        """
    ...
    def get_integration_summary(
        self, datasets: dict[str, pd.DataFrame], master_df: pd.DataFrame
    ) -> dict:
        """
        ...
        for source, df in datasets.items():
            if source in self.data_loaders:
                summary["individual_datasets"][source] = self.data_loaders[source].get_data_summary(
                    df
                )
    ...
    def run_integration(
        self, sources: list | None = None, save_individual: bool = True
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            self.log_info(f"Integration complete. Output files: {list(output_files.keys())}")
...
def main():
    ...
    config = IntegrationConfig(
        raw_data_dir=args.raw_data_dir, output_dir=args.output_dir, year_range=year_range
    )

    # Run integration with refactored pipeline
    integrator = DataIntegrator(config=config)
    master_df = integrator.run_integration(
        sources=args.sources, save_individual=not args.skip_individual
    )
