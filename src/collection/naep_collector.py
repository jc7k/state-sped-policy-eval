...
from .common import SafeTypeConverter
...
class NAEPDataCollector(APIBasedCollector):
    ...
    def _safe_float(self, value) -> float | None:
        ...
        return SafeTypeConverter.safe_float(
            value, invalid_values=[None, "", "null", "‡", "*", "N/A", "#"]
        )
    ...
    def validate_data(self, df: pd.DataFrame) -> dict:
        ...
        validation = {
            ...
            "missing_swd_scores": (
                df["swd_mean"].isna().sum() if len(df) > 0 and "swd_mean" in df.columns else 0
            ),
    ...
    def _get_default_output_path(self) -> str:
        ...
        return self.file_utils.get_default_output_path(
            "naep_raw.csv", env_var="RAW_DATA_DIR", default_dir="data/raw"
        )
