...
class DataValidator:
    ...
    def validate_state_coverage(df: pd.DataFrame, state_column: str = "state") -> dict:
        ...
        if states_covered < 50:
            if states_covered < 2:
                validation["errors"].append(f"Only {states_covered} states covered, expected 50+")
    ...
    def check_duplicates(df: pd.DataFrame, subset: list[str]) -> dict:
        ...
        validation = {"duplicates": duplicates, "passed": duplicates == 0, "errors": []}
    ...
    @staticmethod
    def check_missing_data_rate(df: pd.DataFrame, column: str, threshold: float = 0.3) -> dict:
        """
        ...
        validation = {"missing_rate": missing_rate, "missing_count": missing_count, "warnings": []}

        if missing_rate > threshold:
            validation["warnings"].append(f"High missing data rate in {column}: {missing_rate:.1%}")
