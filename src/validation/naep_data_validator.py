...
class NAEPDataValidator:
    ...
    def _validate_completeness(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        completeness["coverage_by_dimension"] = {
            ...
            "disability_statuses": {
                ...
                "missing": list(
                    set(self.expected_disability_statuses) - set(df["disability_status"].unique())
                ),
                "unexpected": list(
                    set(df["disability_status"].unique()) - set(self.expected_disability_statuses)
                ),
            },
        }
        ...
        for group_by in [None, "year", "grade", "subject", "state"]:
            if group_by is None:
                ...
                for group_value in df[group_by].unique():
                    ...
                    non_swd_scores = group_data[group_data["disability_status"] == "non-SWD"][
                        "mean_score"
                    ]
    ...
    def _check_var_value_consistency(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        mapping_check = (
            df.groupby(["var_value", "disability_status"]).size().reset_index(name="count")
        )
