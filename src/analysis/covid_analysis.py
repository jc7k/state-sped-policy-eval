...
class COVIDAnalyzer:
    ...
    def prepare_triple_diff_data(self, outcome: str) -> pd.DataFrame:
        ...
        analysis_data["covid_period"] = (
            analysis_data["year"].map(lambda x: 1 if x in covid_years_set else 0).astype("int8")
    ...
    def estimate_triple_difference(
        ...
        try:
            ...
            # Interpretation
            if ddd_coeff > 0:
                print("  → Policy reforms WORSENED outcomes during COVID (less resilient)")
            else:
    ...
    def analyze_resilience_by_reform_timing(
        ...
        choices = ["Early Reformer (≤2017)", "Mid Reformer (2018-2020)", "Late Reformer (≥2021)"]
    ...
    def create_covid_analysis_visualization(self, save_formats: list[str] = None) -> str:
