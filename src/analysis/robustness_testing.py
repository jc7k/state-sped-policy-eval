...
class RobustnessTestingSuite:
    ...
    def leave_one_state_out_analysis(
        ...
        baseline_att = (
            baseline_result.get("simple_att", [np.nan])[0] if not baseline_result.empty else np.nan
        )
        ...
        for spec_counter, controls, sample_var in self._generate_spec_combinations():
            ...
            try:
                ...
                if not result.empty and "att" in result.columns:
                    ...
                    ci_lower = (
                        att_estimate - 1.96 * se_estimate if not np.isnan(se_estimate) else np.nan
                    )
                    ci_upper = (
                        att_estimate + 1.96 * se_estimate if not np.isnan(se_estimate) else np.nan
                    )
                    ...
                    significant = (
                        (abs(att_estimate / se_estimate) > 1.96)
    ...
    def _placebo_generator(self, outcome: str, n_placebo: int):
        ...
        for i in range(n_placebo):
            try:
                ...
                placebo_treated_states = np.random.choice(all_states, size=n_treated, replace=False)
    ...
    def placebo_tests(
        ...
        for iteration, placebo_data, n_placebo_treated in self._placebo_generator(
            outcome, n_placebo
        ):
            if (iteration) % 20 == 0:
            ...
            try:
                ...
                # Vectorized significance test
                significant = (
                    (abs(placebo_att / placebo_se) > 1.96) if not np.isnan(placebo_se) else False
                )
        ...
        if not placebo_df.empty:
            ...
            if not valid_placebo.empty:
                placebo_summary = {
                    ...
                    "p_value_empirical": (
                        abs(valid_placebo["placebo_att"]) >= abs(self._get_true_estimate(outcome))
                    ).mean(),
                }
    ...
    def create_robustness_visualization(
        ...
        try:
            ...
            if not jackknife_df.empty:
                ...
                ax1.scatter(range(len(valid_jack)), valid_jack["att_estimate"], alpha=0.6, s=50)
                ax1.axhline(
        ...
        try:
            ...
            if spec_curve_path.exists():
                ...
                summary_text += (
                    f"Significant specs: {spec_df['significant_5pct'].sum()}/{len(spec_df)}\\n"
                )
    ...
    def run_comprehensive_robustness(self, outcomes: list[str] | None = None) -> dict[str, dict]:
